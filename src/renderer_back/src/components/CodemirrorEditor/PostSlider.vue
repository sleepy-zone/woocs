...
async function addPost() {
  ...
  const res = await store.addPost(addPostInputVal.value)
  if (!res) {
    toast.error(`内容新增失败，请更换文件名重试`)
    return
  }
  isOpen.value = false
...
async function renamePost() {
  ...
  const res = await store.renamePost(editTarget.value, renamePostInputVal.value)
  if (!res) {
    toast.error(`内容重命名失败，请更换文件名重试`)
    return
  }
...
async function delPost() {
  const res = store.delPost(editTarget.value)
  if (!res) {
    toast.error(`内容删除失败`)
    return
  }
